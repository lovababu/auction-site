...
public class UserE2ETests extends BaseE2ETest {
    ...
    public void testRegister() {
        AuctionResponse response = registerUser("dpadala1@sapient.com");
    ...
    public void testRegisterWithExistingUserId() {
        ...
        //create user1.
        AuctionResponse response = registerUser("dpadala2@sapient.com");
        assertEquals(response.getStatusCode(), Response.Status.CREATED.getStatusCode());
        assertEquals(response.getMessage(), "User registration successful.");

        //create user2.
        response = registerUser("dpadala2@sapient.com");
    ...
    public void testLogin() {
        //create user1.
        AuctionResponse response = registerUser("dpadala3@sapient.com");
        assertEquals(response.getStatusCode(), Response.Status.CREATED.getStatusCode());
        assertEquals(response.getMessage(), "User registration successful.");

        //Login.
        HttpEntity httpEntity = new HttpEntity(user("dpadala3@sapient.com"), httpHeaders);
    ...
    public void testLoginFailed() {
        UserVO user = user("dpadala4@sapient.com");
        ...
        assertTrue(response.getErrorVOs().size() > 0);
        for (ErrorVO errorVO : response.getErrorVOs()) {
            assertEquals(errorVO.getMessage(), String.format("User not found %s", user.getEmail()));
        }
    }
